...
* The non-greedy operators `*?`, `+?`, `??` and `{m,n}?` are permitted but do nothing, since they do not alter the regular language. For example, `abc{0,5}def` and `abc{0,5}?def` represent precisely the same set of strings.
